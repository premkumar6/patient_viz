...
class OMOP:
    def __init__(self, settings, debug_output):
        ...
        self.schema = settings['omop_schema']
        self.db = create_engine(f'{engine}://{username}:{password}@{host}:{port}/{database}')
        self._parents = {}
        self._codes = {}
        if settings['omop_use_alt_hierarchies']:
            if 'ccs_diag' in settings:
    ...
    def get_patient_data(self, person_source_value):
        query = f"""
        SELECT * FROM {self.schema}.person WHERE person_source_value = '{person_source_value}'
        """
        try:
            result = self.db.execute(query)
            patient_data = [dict(row) for row in result]
            return patient_data
        except Exception as e:
            self.logger.error(f"Error fetching patient data for person_source_value {person_source_value}: {e}")
            raise
