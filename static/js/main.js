// main.js Main Code which runs properly
...
function start() {
  ...
  //   if (patientId) {
  //     // Function to handle file loading based on patient ID
...
//   function selectFileByPatientId(patientId) {
//     busy.setState(jkjs.busy.state.busy);
//     d3.json("/get_patient_data?id=" + encodeURIComponent(patientId), function(error, json_patient) {
//         if (error) {
//             console.error("Error loading patient data:", error);
//             busy.setState(jkjs.busy.state.warn, "Failed to load patient data. Error: " + error.statusText);
//             return;
//         }
        
//         // Load the updated dictionary
//         d3.json(lastDictionaryFile || dictionaryDefault, function(err_dict, json_dictionary) {
//             if (err_dict) {
//                 console.error("Failed loading dictionary:", lastDictionaryFile || dictionaryDefault, err_dict);
//                 busy.setState(jkjs.busy.state.warn, "Invalid dictionary file. Error: " + err_dict.statusText);
//                 return;
//             }
...
//             var error = true;
//             try {
//                 setPidSel(loadPerson(patientId, json_patient, pool, eventList, typeList, linechart, histogram, dictionary, suppl));
//                 busy.setState(jkjs.busy.state.norm);
//                 relayout();
//                 zui.showAll(false);
//                 error = false;
//             } finally {
//                 if (error) {
//                     busy.setState(jkjs.busy.state.warn, "Error while loading patient data.");
//                 }
//             }
//         });
//     });
// }
  ...
  function selectFileByPatientId(patientId) {
    // Replace with the actual logic to fetch the list of files
    d3.text("patients.txt", "text/plain", function(error, data) {
      if (error) {
        console.error("Error loading patients.txt:", error);
      ...
      if (selectedFile) {
        loadFile(selectedFile, lastDictionaryFile || dictionaryDefault, true);
      } else {
        alert('No matching file found for the given patient ID.');
      }
    });
  }
}
